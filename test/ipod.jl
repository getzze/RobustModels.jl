...
λ = 10_000.0
pen2 = RobustModels.RangedPenalties(
    [2:RobustModels.End()], [RobustModels.SquaredL2Penalty(λ)]
)

m00 = fit(LinearModel, form, data)
m01 = fit(RobustLinearModel, form, data, est1; method=:auto, initial_scale=1)
m02 = fit(RobustLinearModel, form, data, est1; method=:auto, initial_scale=1, ridgeλ=λ)
m03 = fit(RobustLinearModel, form, data, est2)
σ = scale(m03)

@testset "Θ-IPOD: L2Loss method, no penalty ($(pen))" for (pen, methods) in zip(
    (nothing, pen1), (nopen_methods, pen_methods)
)
...
@testset "Θ-IPOD: robust loss, no penalty ($(pen))" for (pen, methods) in zip(
    (nothing, pen1), (nopen_methods, pen_methods)
)
    @testset "solver method $(method)" for method in methods
...
@testset "Θ-IPOD: L2Loss method, $(pen_name) penalty" for pen_name in penalties
    ...
    @testset "Omit intercept, data type: $(typeof(A))" for (A, b) in data_tuples
        ...
        m5 = fit(
            IPODRegression, A, b, loss1, sep_pen1; penalty_omit_intercept=false, kwargs...
        )
