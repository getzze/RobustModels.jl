    ...
    @testset "solver method $(method)" for method in methods
        ...
        if isnothing(pen)
            if method in (:chol, :cholesky, :auto)
                @test_throws Exception rlm(
                    X3, y3, est1; method=method, initial_scale=1, dropcollinear=false
                )
            end
        ...
        if isnothing(pen) && method in (:chol, :cholesky, :auto)
            @test_throws Exception ipod(
                X3, y3, loss1, pen; method=method, initial_scale=1, dropcollinear=false
            )
        end
        m2 = ipod(
            X3,
