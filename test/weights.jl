...
@testset "Fit with weights" begin
    ...
    datalin = DataFrame(; x=xlin, y=ylin)
    formlin = @formula(y ~ 1 + x)
    ...
    @testset "weights: $(est)" for est in estimator_list
        ...
        @testset "Model definition: $(model_def)" for model_def in ("arrays", "formula")
            with_arrays = model_def == "arrays"
            if with_arrays
                ...
                m1 = rlm(formlin, datalin[1:r, :], est)
                m1w = rlm(formlin, datalin, est; wts=wlin)

                m2 = rlm(formlin, datalin[(r + 1):end, :], est)
                m2w = rlm(formlin, datalin, est; wts=wwlin)
            end
