...
@testset "Penalties" begin
    ...
    @testset "Methods penalty functions: $(name)" for name in ("No", penalties...)
        ...
        pen = typepenalty(λ)
        penpos = typepenalty(λ; nonnegative=true)
        ...
        @testset "RangedPenalties" begin
            ...
            concpen = RangedPenalties([2:n], [pen], n)
            seppen2 = RangedPenalties([[1], 2:End()], [EuclideanPenalty(λ), pen])

            # Not concrete
            @test !isconcrete(seppen)
            @test isconcrete(concpen)
            ...
            # Convex
            @test isconvex(seppen) == isconvex(pen)
            @test isconvex(concpen) == isconvex(pen)

            @test_throws Exception isseparable(seppen)
            @test isseparable(concpen) == isseparable(pen)
            @test_throws Exception isseparable(seppen2)
    ...
    @testset "M-estimator with penalty: Ridge" begin
        ...
        m0 = rlm(form2, data2, MEstimator{L2Loss}(); kwargs...)
        m1 = rlm(form2, data2, MEstimator{L2Loss}(); ridgeλ=λ, kwargs...)
        ...
        @testset "solver method $(method)" for method in pen_methods
            ...
            @testset "data type: $(typeof(A))" for (A, b) in data2_tuples
                ...
                @testset "method: $(f)" for f in interface_methods
                    # make sure the methods for IPODRegression give the same results
    ...
    @testset "M-estimator with penalty $(name)" for name in penalties
        ...
        pen = typepenalty(λ)
        penpos = typepenalty(λ; nonnegative=true)
        kwargs = (; initial_scale=1, maxiter=1000)
        def_rtol = 1e-3

        m1 = fit(RobustLinearModel, form2, data2, pen; verbose=false, kwargs...)
        ...
        @testset "method: $(method)" for method in pen_methods
            m2 = fit(RobustLinearModel, form2, data2, pen; method=method, kwargs...)
            rtol = method == :fista ? 1e-1 : def_rtol
            @test isapprox(coef(m1), coef(m2); rtol=rtol)
        end

        m3 = fit(RobustLinearModel, form2, data2, penpos; verbose=false, kwargs...)
