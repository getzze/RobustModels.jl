...
function StatsAPI.dof(r::RobustLinearModel{T,R,P}) where {T,R,P<:AbstractRegularizedPred}
    return min(wobs(r), dof(penalty(r), coef(r)))
end
...
function StatsAPI.fit(
    ...
    elseif method in (:cgd, :auto)
        CGDRegPred(X, new_penalty, wts, outerloop)
    else
...
function init!(
    m::RobustLinearModel{T,R,P}; verbose::Bool=false
) where {T,R,P<:AbstractRegularizedPred}
...
function setη!(
    m::RobustLinearModel{T,R,P}, linesearch_f::Real=1; verbose::Bool=false
) where {T,R,P<:AbstractRegularizedPred}
    ...
    if m.pred isa CGDRegPred
        ...
        # Update coefs, μ and residuals
        update_βμ!(m.pred, y, μ, res, σ2, wts; verbose=verbose)

        # Update deviance with the new residuals
        map!(abs2, devresid, res)
    else
        # Update coefs
