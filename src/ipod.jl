...
function initpred!(
    p::LinPred, wt::AbstractVector{T}=T[], σ::Real=one(T); verbose::Bool=false
) where {T<:BlasReal}
    return p
end
...
function solvepred!(
    p::LinPred, r::AbstractVector{T}, wts::AbstractVector{T}
) where {T<:BlasReal}
    return delbeta!(p, r, wts)
end
...
function solvepred!(
    p::Union{DensePredCG{T},SparsePredCG{T}}, r::AbstractVector{T}
) where {T<:BlasReal}
...
"""
mutable struct IPODResp{T<:AbstractFloat,V<:AbstractVector{T},L<:LossFunction} <:
               RobustResp{T}
    "`loss`: loss used for the model"
    loss::L
    "`y`: response vector"
    ...
    function IPODResp{T,V,L}(
        l::L, y::V, precision::V, outliers::V, σ::Real
    ) where {L<:LossFunction,V<:AbstractVector{T}} where {T<:AbstractFloat}
        n = length(y)
        ll = length(precision)
        ll == 0 ||
            ll == n ||
            throw(DimensionMismatch("length of precision is $ll, must be $n or 0"))
        σ > 0 || throw(ArgumentError("σ must be positive: $σ"))
        wrky = y - outliers
        return new{T,V,L}(
            l, y, zeros(T, n), precision, outliers, convert(T, σ), wrky, copy(wrky)
        )
    end
end
...
"""
function IPODResp(
    l::L, y::V, precision::V=T[], outliers::V=zeros(T, length(y)), σ::Real=1
) where {L<:LossFunction,V<:AbstractVector{T}} where {T<:AbstractFloat}
...
"""
function IPODResp(
    l::L, y, precision=eltype(y)[], outliers=zeros(eltype(y), length(y)), σ::Real=1
) where {L<:LossFunction}
    return IPODResp(
        l, float(collect(y)), float(collect(precision)), float(collect(outliers)), σ
    )
end
...
function initresp!(r::IPODResp)
    ...
    return broadcast!(-, r.wrkres, r.wrky, r.μ)
end
...
function outliers_criteria(r::IPODResp{T}, γold::AbstractVector{T}) where {T<:AbstractFloat}
    return maximum(abs(γold[i] - r.outliers[i]) / r.σ for i in eachindex(γold, r.outliers))
end
...
function IPODRegression(
    ...
    # Method
    nopen_methods = (:auto, :chol, :cholesky, :cg, :qr)
    pen_methods = (:auto, :cgd, :fista, :ama, :admm)
    if isnothing(penalty) && method ∉ nopen_methods
        @warn(
            "Incorrect method `:$(method)` without a penalty, should be one of $(nopen_methods)"
        )
        method = :auto
    elseif !isnothing(penalty) && method ∉ pen_methods
        @warn(
            "Incorrect method `:$(method)` with a penalty, should be one of $(pen_methods)"
        )
        method = :auto
    end
    ...
    if isnothing(penalty)
        ...
        elseif method in (:chol, :cholesky, :auto)
            ...
            error(
                "without penalty, method :$method is not allowed, should be in: $nopen_methods",
            )
        end
        ...
        pred = if method === :fista
            FISTARegPred(X, new_penalty, wts, restart, use_backtracking)
        elseif method === :ama
...
function StatsAPI.dof(r::IPODRegression)
    df = if haspenalty(r)
        dof(penalty(r), coef(r))
    else
        length(coef(r))
    end
    return min(wobs(r), df)
end
...
function GLM.dispersion(r::IPODRegression, sqr::Bool=false)
    wts = weights(r)
    res = residuals(r)
    dofres = dof_residual(r)

    if dofres <= 0
        return convert(eltype(res), Inf)
    end
...
function penalty(r::IPODRegression{T,R,P,V}) where {T,R,P<:AbstractRegularizedPred,V}
    return r.pred.penalty
end
...
function update_fields!(
    ...
    if !isnothing(λ0)
        ...
        elseif length(λ0) in (0, n)
            ...
            throw(
                ArgumentError("λ0 should be a real or a vector of length 0 or $n: $(λ0))")
            )
        end
    end
    if !isnothing(β0)
...
function StatsAPI.fit(
    ...
    f, y, X, extra = modelframe(
        f, data, contrasts, dropmissing, M; wts=wts, precision=precision
    )
...
function StatsAPI.fit!(
    ...
    maxiter::Integer=(P <: FISTARegPred) ? 1000 : 100,
...
function refit!(m::IPODRegression; method=nothing, kwargs...)
...
function init!(m::IPODRegression{T}; verbose::Bool=false) where {T<:AbstractFloat}
...
function setη!(
    m::IPODRegression{T,R,P,V}, linesearch_f::Real=1; verbose::Bool=false
    ...
    return update_residuals!(m.resp)
end
...
function setη!(
    ...
    if m.pred isa CGDRegPred
        ...
        # Update coefs and μ
        update_βμ!(m.pred, wrky, μ, res, σ2, wts; verbose=verbose)
